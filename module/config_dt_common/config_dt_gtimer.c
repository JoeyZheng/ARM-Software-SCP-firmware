...
#define GTIMER_COMPAT scp_gtimer
...
static const struct fwk_element gtimer_element_table[] = {
    ...
    DT_FOREACH_OKAY_INST_scp_gtimer(CFG_GTIMER_ELEM_INIT)
    // last null element
    [DT_N_INST_scp_gtimer_NUM_OKAY] = { 0 },
};
...
struct fwk_module_config config_dt_gtimer = {
    ...
    #error("GTIMER module does not support DT dynamic configuration")
    #else
