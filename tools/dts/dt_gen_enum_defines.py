...
def cxx_strip_int_ul(int_str):
    # strip off UL / U from int declarations
    out_str = int_str;
    ulpos = int_str.find('UL')
    if ulpos > 0:
        out_str = int_str[0:ulpos]
        if cxx_debug:
            print(f"stripped UL: {int_str} => {out_str}, ulpos={ulpos}")
    else:
        upos = int_str.find('U')
        if upos > 0:
            out_str = int_str[0:upos]
            if cxx_debug:
                print(f"stripped U: {int_str} => {out_str}, upos={upos}")
...
def cxx_eval_enum_val(enum_val):
    ...
    for etoken in enum_val.value.tokens:
        # if cxx_debug:
        #    print(f"ETOKEN = {etoken}, type = {etoken.type}")

        # ints might have a UL / U suffix - this breaks eval so strip
        if etoken.type == 'INT_CONST_DEC':
            etoken.value = cxx_strip_int_ul(etoken.value)
